class CompatibilityCheck
  ...
  def initialize(params = {})
    ...
    @part_attributes = params[:part_attributes].delete_if { |x| x.empty? }
  end
  ...
  def process
    if @part_attributes.blank?
      ...
      vehicle_one_parts = @vehicle_one.oem_parts.joins(:product, :part_attributes).where('products.product_type_id = ? AND part_attributes.id IN (?)', @product_type.id, @part_attributes).includes(product: :brand)
      vehicle_two_parts = @vehicle_two.oem_parts.joins(:product, :part_attributes).where('products.product_type_id = ? AND part_attributes.id IN (?)', @product_type.id, @part_attributes).includes(product: :brand)
    end
