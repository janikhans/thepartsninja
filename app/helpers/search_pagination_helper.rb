module SearchPaginationHelper
  ...
  def paginate_check_search(search_record, options = {})
    ...
    nav = '<ul class="search-pagination pagination pull-right">'
    nav += first_check_page(search_record) unless search_record.current_page == 1
    nav += previous_check_page(search_record) unless search_record.current_page == 1
    (page_start..page_end).to_a.each do |page|
      next if page > search_record.total_pages
      if search_record.current_page == page
        nav += content_tag(:li, content_tag(:span, page), class: "active")
      else
        nav += content_tag(:li, link_to(page,check_path(search_record, page: page), remote: true))
      end
    end
    nav += next_check_page(search_record) unless search_record.current_page == search_record.total_pages
    nav += last_check_page(search_record) unless search_record.current_page == search_record.total_pages
    nav += "</ul>"
    nav.html_safe
  end
  ...
  def previous_check_page(search_record)
    content_tag(:li, link_to(icon(:angle_left), check_path(search_record, page: search_record.current_page - 1), remote: true))
  end

  def next_check_page(search_record)
    content_tag(:li, link_to(icon(:angle_right), check_path(search_record, page: search_record.current_page + 1), remote: true))
  end

  def first_check_page(search_record)
    content_tag(:li, link_to(icon(:angle_double_left), check_path(search_record, page: 1), remote: true))
  end

  def last_check_page(search_record)
    content_tag(:li, link_to(icon(:angle_double_right), check_path(search_record, page: search_record.total_pages), remote: true))
  end
end
